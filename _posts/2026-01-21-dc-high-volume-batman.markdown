---
layout: post
category: Audio Dramas
title: DC High Volume: Batman - Review
date: 2026-01-21
---

## Overview

<div class="review-overview">
An official adaptation of the comics Batman: Year One by writer Frank Miller and artist David Mazzucchelli, and Batman: The Long Halloween, and Batman: Dark Victory by writer Jeph Loeb and artist Tim Sale.
</div>

## Review

<div class="review-criteria">
<h3><span class="review-score-label">Soundscape / Production:</span> <span class="review-score">4.5/5</span></h3>
<p>As you'd expect (or at least hope for) from the official production of something as big as Batman, DC does a great job with production.</p>
</div>

<div class="review-criteria">
<h3><span class="review-score-label">Voice Acting:</span> <span class="review-score">5/5</span></h3>
<p>Voice acting is really good as well. All are legit voice actors and I don't see a reason to give it anything under a 10</p>
</div>

<div class="review-criteria">
<h3><span class="review-score-label">Writing:</span> <span class="review-score">4/5</span></h3>
<p>It's pretty easy to come up with a good script when you're adapting an already proven story.</p>
</div>

<div class="review-criteria">
<h3><span class="review-score-label">Story:</span> <span class="review-score">4/5</span></h3>
<p>I was never the biggest superhero movie fan. Like some of them were okay, but I was always more into sci-fi. But this story is really good and it's easy want to listen to the next episode</p>
</div>

<div class="final-score">
<div class="final-score-label">87.5</div>
<div class="final-score-value">17/20</div>
</div>

<div class="review-notes">
<h4>Link to Audio Drama</h4>
<p>https://www.dc.com/highvolume</p>
</div>

